view: affinity {
  derived_table: {
    ...
    FROM (
      ...
      SELECT
        store_id, product_id_1, product_id_2, SUM(quantity1) AS how_many1, SUM(quantity2) AS how_many2
      FROM (

        -- find customer order baskets
        SELECT t1.store_id, t1.product_id AS product_id_1, t2.product_id AS product_id_2, t1.order_date, t1.customer_id, t1.quantity as quantity1, t2.quantity as quantity2
        FROM  (
          SELECT distinct store_id, product_id, order_date, customer_id, quantity FROM lineitem
        ) AS t1
        JOIN  (
          SELECT distinct store_id, product_id, order_date, customer_id, quantity FROM lineitem
        ) AS t2
      ...
      GROUP by t1.store_id, t1.product_id_1, t1.product_id_2
